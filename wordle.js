...
const getYellowValues = () => { // retrieve yellow values
    for (y = 1; y < 6; y++) {
        if(document.getElementById("Y" + y).value.length > 0){
            yArr.push(document.getElementById("Y" + y).value)
        } else {
            yArr.push("")
        }    }
};

const getGreyValues = () => { // retrieve grey values
...
const yellowTestNoMatch = () => { // check yellow letters aren't at that position
...
const wordMatch = () => {
    ...
    for (i = 0; i < matchList.length; i++) {  // loop through each word
        ...
        if (yellowTestPos(matchList[i])) continue; // if the yellow letters exist elsewhere
